...
namespace Extrude.Framework.Graphics
{
	...
	#region Public Properties
		...
		public Rectangle Bounds {
			get {
				return new Rectangle ((int)positionValue.X, (int)positionValue.Y, SpriteWidth, SpriteHeight);
			}
		}
		...
		public void ActivateDebugMode (Texture2D debugTexture)
		{
			_debugMode = true;
	...
	#region Methods
		...
		public void Draw (SpriteBatch batch, Color color, BlendState blendMode)
		{
			...
			if (_debugMode && _debugTexture != null) {
				batch.Draw (_debugTexture, Bounds, sheet [currentFrameValue], color, rotationValue, originValue, SpriteEffects.None, 1f);
				//Console.WriteLine ("DrawingDebug!!");
			}
			//batch.Draw(sheet.Texture,positionValue,sheet [currentFrameValue],color,rotationValue,)
			
			
			//batch.Draw (sheet.Texture, positionValue, sheet [currentFrameValue], color, rotationValue, originValue, 1f, SpriteEffects.None, 100f);
			batch.Draw (sheet.Texture, Bounds, sheet [currentFrameValue], color, rotationValue, originValue, SpriteEffects.None, 1f);
		
			// funkar
			//batch.Draw (sheet.Texture, new Rectangle ((int)positionValue.X, (int)positionValue.Y, 100, 100), sheet [currentFrameValue], color);
