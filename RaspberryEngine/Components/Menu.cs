    ...
    public class Menu
    {
            ...
            for (int s = 0; s < optionsMenu.Count; s++)
            {
                ...
                switch (align)
                {
                    case MenuAlign.Center:
                        orginX = ((SpriteFont)screen.ScreenManager.GetAsset(fontKey)).MeasureString(optionsMenu[s]).X / 2 - bound.Width / 2;
                        break;
                    case MenuAlign.Right:
                        orginX = ((SpriteFont)screen.ScreenManager.GetAsset(fontKey)).MeasureString(optionsMenu[s]).X - bound.Width;
                        break;
                    default:
                        orginX = 0;
                        break;
        ...
        public void AddOptions(Screen screen, string[] newMenuOptions)
        {
            ...
            foreach (string s in newMenuOptions)
            {
                ...
                Vector2 measure = ((SpriteFont)screen.ScreenManager.GetAsset(fontKey)).MeasureString(s.ToUpper());
